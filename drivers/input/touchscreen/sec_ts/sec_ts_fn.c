...
static void run_cmoffset_hf_read(void *device_data);
static void run_cmoffset_hf_read_all(void *device_data);
...
static void run_raw_p2p_key_min_read_all(void *device_data);
static void run_raw_p2p_key_max_read_all(void *device_data);
static void run_cmoffset_key_read(void *device_data);
static void run_cmoffset_key_read_all(void *device_data);
...
static void get_cmoffset_hf(struct sec_ts_data *ts, struct sec_cmd_data *sec, struct sec_ts_test_mode *mode);
static void sec_ts_get_key_cmoffset(struct sec_ts_data *ts,
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("run_cmoffset_hf_read", run_cmoffset_hf_read),},
	{SEC_CMD("run_cmoffset_hf_read_all", run_cmoffset_hf_read_all),},
	...
	{SEC_CMD("run_raw_p2p_key_min_read_all", run_raw_p2p_key_min_read_all),},
	{SEC_CMD("run_raw_p2p_key_max_read_all", run_raw_p2p_key_max_read_all),},
	{SEC_CMD("run_cmoffset_key_read", run_cmoffset_key_read),},	
	{SEC_CMD("run_cmoffset_key_read_all", run_cmoffset_key_read_all),},
...
#if 0// defined(USE_PRESSURE_SENSOR) 
		...
		short *max, bool save_result)
{
	...
	input_raw_info(true, &ts->client->dev, "%s: type %d, rx %d, tx %d, readbytes %d\n",
			__func__, type, ts->tx_count, ts->rx_count, readbytes);
				...
				short *min, short *max, bool save_result)
{
	...
	input_raw_info(true, &ts->client->dev, "%s: type %d, tx %d, tx %d = %d\n",
			__func__, type, ts->tx_count, ts->rx_count, data_length);
...
static int get_gap_data(void *device_data)
{
	...
	char buff[16] = { 0 };
	int ii;
	signed int node_gap_tx = 0;
	signed int node_gap_rx = 0;
	signed int tx_min = 0, tx_max = 0;
	signed int rx_min = 0, rx_max = 0;

	for (ii = 0; ii < (ts->rx_count * ts->tx_count); ii++) {

		if (ii < ts->rx_count * (ts->tx_count - 1)) {
			node_gap_tx = ts->pFrame[ii] - ts->pFrame[ii + ts->rx_count];
			if (ii == 0)
			...
			tx_max = max(tx_max, node_gap_tx);
				...
				input_dbg(true, &ts->client->dev,
					...
					__func__, ii, ts->pFrame[ii] - ts->pFrame[ii + ts->rx_count],
					ii, ts->pFrame[ii] , ii + ts->rx_count, ts->pFrame[ii + ts->rx_count]);
		}

		if ((ii + 1) % (ts->rx_count) != 0) {
			node_gap_rx = ts->pFrame[ii] - ts->pFrame[ii + 1];
			if (ii == 0)
			...
			input_dbg(true, &ts->client->dev,
				...
				__func__, ii, ts->pFrame[ii] - ts->pFrame[ii + 1], ii, ts->pFrame[ii], ii + 1,ts->pFrame[ii + 1]);
		}
...
static void get_gap_data_x_all(void *device_data)
{
	...
	for (ii = 0; ii < (ts->rx_count * ts->tx_count); ii++) {
		if ((ii + 1) % (ts->tx_count) != 0) {
			node_gap = ts->pFrame[ii] - ts->pFrame[ii + 1];
...
static void get_gap_data_y_all(void *device_data)
{
	...
	for (ii = 0; ii < (ts->rx_count * ts->tx_count); ii++) {
		if (ii < (ts->rx_count - 1) * ts->tx_count) {
			node_gap = ts->pFrame[ii] - ts->pFrame[ii + ts->tx_count];
		...
		struct sec_cmd_data *sec, struct sec_ts_test_mode *mode)
{
	...
	if (mode->key) {
		ret = sec_ts_key_rawdata_read(ts, mode->type, &mode->min, &mode->max);
		if (ret < 0) {
			input_err(true, &ts->client->dev, "%s: failed to read key data\n",
					__func__);
			goto error_test_fail;
		}

		if (mode->allnode) {
			for (ii = 0; ii < ts->plat_data->support_tsk; ii++) {
				snprintf(temp, CMD_RESULT_WORD_LEN, "%d,", ts->pFrame[ii]);
				strncat(buff, temp, CMD_RESULT_WORD_LEN);
		...
		struct sec_cmd_data *sec)
{
	...
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING) {
		...
		snprintf(buff, sizeof(buff), "%d,%d", ts->cm_raw_key_p2p_min, ts->cm_raw_key_p2p_max);
		if (ts->plat_data->support_tsk) {
			sec_cmd_set_cmd_result_all(sec, buff, strnlen(buff, sizeof(buff)), "CM_RAW_KEY_P2P");
			snprintf(buff, sizeof(buff), "%d,%d", 0, ts->cm_raw_key_p2p_diff);
			sec_cmd_set_cmd_result_all(sec, buff, strnlen(buff, sizeof(buff)), "CM_RAW_KEY_P2P_DIFF");
		}
	}
...
static void run_raw_p2p_read_all(void *device_data)
{
	...
	/* read both mutual, self for factory_cmd_result_all*/
	sec_ts_read_rawp2p_data_all(ts, sec);
}
...
static void run_cmoffset_key_read(void *device_data)
{
	...
	sec_ts_get_key_cmoffset(ts, sec, &mode);
}
...
int sec_tclm_data_read(struct i2c_client *client, int address)
{
	...
	case SEC_TCLM_NVM_OFFSET_IC_FIRMWARE_VER:
		...
		if (ret < 0)
			return ret;
		input_err(true, &ts->client->dev, "%s SEC_TS_READ_IMG_VERSION buff[2]:%d buff[3]:%d \n",
...
static int execute_selftest(struct sec_ts_data *ts, bool save_result)
{
	...
	input_info(true, &ts->client->dev, "%s: Self test start![0x%X]\n", __func__, tpara[0]);
...
static void get_cmoffset_hf(struct sec_ts_data *ts,
		struct sec_cmd_data *sec, struct sec_ts_test_mode *mode)
{
	/* fix length: if all channel failed, can occur overflow */
	char *buff;
	int rc;
	int size = ts->tx_count * ts->rx_count * CMD_RESULT_WORD_LEN;
	char para = TO_TOUCH_MODE;
	u8 *rBuff = NULL;
	char temp[SEC_CMD_STR_LEN] = { 0 };
	int ii;
	...
	if (ts->power_status == SEC_TS_STATE_POWER_OFF) {
		input_err(true, &ts->client->dev, "%s: Touch is stopped!\n", __func__);
		snprintf(temp, sizeof(temp), "%s", "TSP turned off");
	...
	for (ii = 0; ii < (ts->tx_count * ts->rx_count * 2); ii += 2){
		ts->pFrame[ii / 2] = (rBuff[80 + ii + 1] << 8) + rBuff[80 + ii];
	}
	...
	if (mode->allnode) {
		...
		snprintf(buff, SEC_CMD_STR_LEN, "%d,%d", min, max);
	}

	input_info(true, &ts->client->dev, "%s: #29 min/max [%d/%d]\n", __func__, min, max);
	...
	if (!sum) {
		input_info(true, &ts->client->dev, "%s: Check 80 byte is OK!\n", __func__);
	} else{
		input_info(true, &ts->client->dev, "%s: Check 80 byte is Fail\n", __func__);
	}

	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, ts->tx_count * ts->rx_count * CMD_RESULT_WORD_LEN));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buff, strnlen(buff, sizeof(buff)), "CM_OFFSET_HF");
...
out:
	...
	sec_cmd_set_cmd_result(sec, temp, strnlen(temp, sizeof(temp)));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, temp, strnlen(temp, sizeof(temp)), "CM_OFFSET_HF");
...
#endif
	else if (cal_mode == OFFSET_CAL_TMP)
		cmd = SEC_TS_CMD_TMP_PANELCALIBRATION;
	else
		return rc;
		...
		u8 type, short *min, short *max)
{
	...
	/* set data length, allocation buffer memory */
	readbytes = ts->plat_data->support_tsk * 2;

	input_info(true, &ts->client->dev, "%s: readbytes[%d]\n", __func__, readbytes);

	pRead = kzalloc(readbytes, GFP_KERNEL);
	if (!pRead)
		return -ENOMEM;

	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_KEY_RAW_MODE, &type, 1);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: tsk cmoffet type write failed(%d)\n", __func__, ret);
		goto out;
	}
	...
	if (type == TYPE_OFFSET_DATA_SDC) {
		...
		char para = TO_TOUCH_MODE;
		disable_irq(ts->client->irq);
		ret = execute_selftest(ts, true); // for 0x27 save flash
		if (ret < 0) {
			input_err(true, &ts->client->dev, "%s: execute_selftest failed!\n", __func__);
			enable_irq(ts->client->irq);
			goto out;
	...
	/* read data */
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_KEY_RAW_VALUE, pRead, readbytes);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: read tsk cmoffet failed!\n", __func__);
		goto out;
	}
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_KEY_RAW_MODE, &type, 1);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: tsk cmoffet 0xff write failed(%d)\n", __func__, ret);
		goto out;
	}
	...
	for (i = 0 ; i < readbytes ; i += 2) {
		ts->pFrame[i / 2] = (pRead[i] << 8) + pRead[1 + i];
		
		if (i == 0) {
			*min = *max = ts->pFrame[i / 2];
		} else {
			*min = min(*min, ts->pFrame[i / 2]);
			*max = max(*max, ts->pFrame[i / 2]);
		}
	}
out:
	return ret;
		...
		struct sec_cmd_data *sec, struct sec_ts_test_mode *mode)
{
	...
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, ts->plat_data->support_tsk * CMD_RESULT_WORD_LEN));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buff, strnlen(buff, sizeof(buff)), "CM_OFFSET_KEY");
...
static s16 sec_ts_get_key_sensitivity(struct sec_ts_data *ts, int index_num)
{
	char buff[20] = { 0 };
	u8 read_val[4] = { 0 };
	s16 ret_val = 0;
	...
	read_val[0] = 1;
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_KEY_RAW_MODE, read_val, 1);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: threshold write type failed. ret: %d\n", __func__, ret);
		snprintf(buff, sizeof(buff), "%s", "NG");
		return ret;
	...
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_KEY_RAW_VALUE, read_val, 4);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: read threshold fail!\n", __func__);
		snprintf(buff, sizeof(buff), "%s", "NG");
		return ret;
	...
	input_info(true, &ts->client->dev, "%s : 0x%02X, 0x%02X || 0x%02X, 0x%02X\n",
				__func__, read_val[0], read_val[1], read_val[2], read_val[3]);

	ret_val = (read_val[0 + index_num * 2] << 8) | read_val[1 + index_num * 2];
...
static ssize_t recent_sensitivity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sensitivity = sec_ts_get_key_sensitivity(ts, 0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	sensitivity = sec_ts_get_key_sensitivity(ts, 1);
		...
		struct device_attribute *attr, char *buf)
{
	...
	input_info(true, &ts->client->dev, "%s : 0x%02X, 0x%02X\n",
...
err:
#endif
	return sprintf(buf, "%d", 200);
}

/* sysfs: /sys/class/sec/sec_touchkey/touchkey_irq_count */
static ssize_t touchkey_irq_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_ts_data *ts = dev_get_drvdata(dev);
	int home_count = 0;

	input_info(true, &ts->client->dev, "%s - Recent : %d, Home : %d, Back : %d \n", __func__, ts->irq_key_count[0], home_count, ts->irq_key_count[1]);
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n", ts->irq_key_count[0], home_count, ts->irq_key_count[1]);
}

static ssize_t touchkey_irq_count_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == 48) {
		ts->irq_checked = false;
	} else if (buf[0] == 49) {
		ts->irq_checked = true;
	} else {
		input_err(true,&ts->client->dev, "%s [ERROR] Unknown value [%c]\n", __func__, buf[0]);
		return count;
	}

	if (ts->irq_checked) {
		for (i = 0; i < SUPPORT_TSK_NUM; i++)
			ts->irq_key_count[i] = 0;
	}
...
static DEVICE_ATTR(touchkey_irq_count, 0664, touchkey_irq_count_show, touchkey_irq_count_store);
...
int sec_ts_fn_init(struct sec_ts_data *ts)
{
	...
	if (ts->plat_data->support_tsk) {
		ts->sec_touchkey = sec_device_create(ts, "sec_touchkey");
		if (IS_ERR(ts->sec_touchkey)) {
			input_err(true, &ts->client->dev, "Failed to create device (sec_touchkey)\n");
			goto exit;
		}
		/* /sys/class/sec/sec_touchkey/... */
		if (sysfs_create_group(&ts->sec_touchkey->kobj, &sec_tkey_attr_group)) {
			input_err(true, &ts->client->dev, "Failed to create sysfs group(sec_touchkey)\n");
			goto exit;
		}
	}
